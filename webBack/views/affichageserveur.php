...
if(isset($_POST['tri']))
{
	$liste=$db->query("SELECT * FROM `serveurs` ORDER BY `serveurs`.`nom` ASC");
}
else if(isset($_POST['tria']))
{
	$liste=$db->query("SELECT * FROM `serveurs` ORDER BY `serveurs`.`nom` DESC");
}
else $liste=$db->query("SELECT * FROM `serveurs`");
...
<script >
    $(function () {
        $("#search").keyup(function () {
            $.ajax({
                url : "http://localhost/webBack/views/affichageserveur.php",
                data : { 'searching' : $(this).val() }
            }).done(function (response) {
                var tableau = $(response).find("table");
                $('.w3l-table-info').empty();
                $('.w3l-table-info').append(tableau);
            });
        });
    })
</script>
